...
import debounce from '../utils/debounce'; // Ajusta la ruta según tu estructura de carpetas
 ...
 */
function useLocalStorage<T>(key: string, initialValue: T, delay: number = 300) {
    ...
    const setValue = useCallback(
        debounce((value: T | ((val: T) => T)) => {
            if (typeof window === 'undefined') {
                console.warn(`Cannot set value in localStorage for key '${key}' because window is not defined.`);
                return;
            }
        ...
        const handleStorageChange = (event: StorageEvent) => {
            if (event.key === key) {
                try {
                    ...
                    console.error(`Error parsing new value from localStorage for key '${key}':`, error);
                }
            }
        };
